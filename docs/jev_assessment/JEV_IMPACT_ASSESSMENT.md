...
| K3 | Wrong clearances traced to wrong facts are at least as common as an LLM judge's wrong clearances on the same cases | The experiment in section 7, then the partner's first 100 escalated cases with a human fact sheet |
| K4 | No primitive reaches 90% abstention recall on facts genuinely absent from the record at decision-time state, while the partner's grants need non-system facts | Section 7, plus a fact inventory at the partner classifying each grant's required facts by tier |
...
K4 and K5 are new. K4 is technical and Jev-era: it says the fact layer cannot be trusted to fail closed. K5 is economic and is the packet's open question 1 turned into a number.

---

## 7. Highest-value next experiment

The decisive uncertainty is economic and needs a design partner. That is not a two-week experiment. The best two-week experiment is the one that settles the fact layer so that the partner measurement isolates the economic question, and that operationalises `DECISION.md`'s third build-nothing criterion, which was never given a measurement route.

**Name.** Fact-extraction fidelity with abstention on the 46 IPSA reviews.

**Hypotheses** (falsified if every primitive fails):

- H1, fidelity: on facts the record contains, at least one primitive reaches 95% agreement with a frozen human fact sheet per predicate.
- H2, abstention: on facts genuinely absent from the record, at least one abstention form reaches recall of 90% at precision of 70%.
- H3, kernel worth: on the bright-line-decidable subset, a deterministic rule fed by model-extracted facts at decision-time state produces fewer than half the wrong clearances of a frontier LLM judge deciding the same cases end to end.

**Input data.** The 46 IPSA Compliance Officer PDFs, re-fetched from the URLs in `evidence/sophon_cases/corpus_inventory.json` and verified against its SHA-256 values. The two codings in `corpus_study/coded/` for stratification only. A new human fact sheet, written before any model call and frozen by hash in the style of `results/forecast.sha256`: for each document and each of nine pre-declared predicates, the value, the evidence status (PRESENT_AT_SUBMISSION, PRESENT_ONLY_AT_REVIEW, ABSENT) and the paragraph reference. Predicates: evidence supplied with the submission; submitted within deadline; amount claimed; expense category; purpose was parliamentary; prior approval sought; a cheaper alternative was available; claimant relied on staff advice; an exception was applied for. Two states per document: T0, only the paragraphs describing the original submission and decision; T1, the full document. Facts marked PRESENT_ONLY_AT_REVIEW are absent at T0 by construction, which yields the genuinely absent set without inventing text. The 16 later-facts documents guarantee that set is not empty.

**Implementation.** Days 1 to 2: hash a pre-registration with predicates, three paraphrases per predicate, abstention forms, thresholds and the analysis script. Days 3 to 6: build and freeze the fact sheet, about 45 minutes per document. Day 7: harness. Primitives, all pinned and logged per call: Jev as the explicit id `jev-1.13.0` on TypeSafe's endpoint, rejecting any response whose model field differs; a frontier LLM by exact model id with structured outputs and the document as a cached prefix; a zero-shot NLI encoder pinned by commit hash and run locally; deterministic code for amounts and dates. Three question forms per predicate: binary with abstention only from a 0.35 to 0.65 band; three-way choice with "cannot be determined from this text"; an evidence-presence gate before the binary. Runs: Jev, all forms, 3 paraphrases, 3 repeats at least 12 hours apart; the LLM, the three-way form with 3 paraphrases and 3 repeats, the others once; the encoder once. Days 8 to 10: analysis and write-up, reporting every number that fails.

**Comparison baselines.** The frontier LLM at the same questions; the NLI encoder; deterministic code on numeric facts, which is the floor; and an end-to-end LLM judge on T0 against the pipeline of model facts plus a deterministic rule. Within-model: confidence band versus explicit option versus presence gate; T1 versus T0.

**Metrics.** Per predicate and primitive: accuracy on PRESENT items at T0 and T1 with Wilson intervals; abstention precision and recall on ABSENT items, recall being the safety number; wrong clearances on the bright-line subset, pipeline versus judge, McNemar; ECE and Brier before and after per-predicate fit on a held-out half, reported as a multiple of the simulated noise floor; flip rate across repeats and across paraphrases; a count of true items assigned probability 0.00; cost and latency recorded but not thresholded.

**Pass and fail thresholds.** H1 passes if some primitive reaches 95% on PRESENT items for every predicate a grant would consume, with numeric predicates at 100% from code; it fails if none exceeds 90% on any non-numeric predicate. A grant rests on two or three facts, so 95% each is already a 10 to 14% joint error against a 0.3% base rate of wrong claims. H2 passes at recall 90% and precision 70%; fails below 75% recall for every form. Over-abstention costs an escalation; under-abstention clears an action on an invented fact. H3 passes if pipeline wrong clearances are under half the judge's; fails if at least equal. Calibration usable for routing if held-out ECE is at most twice the noise floor after fit. Paraphrase flips above 10% are a design rule, not a kill: the question text becomes a version-pinned ratified artefact.

**Decision by result.**

- All three pass for at least one primitive: extend convention 7 with a fourth class, model-read from a record the agent did not author with verified abstention, admissible only for predicates that passed; choose the cheapest passing primitive; proceed to the design partner with the fact layer settled.
- H2 fails: models manufacture facts when the record is silent. Convention 7 stays as written. Autonomous coverage is bounded by bright-line facts. The thin layer's whole value is the settlement workflow, and the partner must show recurrence without model-supplied facts.
- H3 fails: `DECISION.md`'s third build-nothing criterion is met. Stop the ledger as an autonomy product. If anything survives it is the decision log with provenance.
- H1 passes at T1 but fails at T0: A2 is confirmed in its strongest form. The facts a grant needs are not in the record at decision time and no extractor helps. The product question becomes whether REQUEST_FACT can be pushed to the claimant at submission, a form-design problem where Sophon's delta over a policy agent is small.

**Cost and time.** About ten working days for one engineer. Under 100 dollars of API spend expected, capped at 300. No partner, no new data collection, no product code.

**What it cannot decide.** Recurrence and prospective settlement. Those stay open until a design partner is instrumented, and they decide whether Sophon exists.
